...
class OrderDetailBloc extends SafeBloc<OrderDetailEvent, OrderDetailState> {
  ...
  OrderDetailBloc({required this.getOneOrderUseCase}) : super(OrderDetailInitialState()) {
  ...
  Future<void> _onLoadOrderDetail(
    ...
    try {
      final orderDetail = await getOneOrderUseCase.execute(GetOneOrderUseCaseInput(orderId: event.orderNumber));
